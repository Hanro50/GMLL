...
import type { DownloadableFile, RuntimeManifestEntry, RuntimeManifest, MCRuntimeVal, VersionJson, AssetIndex, Artifact, MojangResourceManifest, MojangResourceFile, VanillaManifestJson } from "../types";
    ...
    Object.keys(json).forEach(key => {
        ...
        switch (obj.type) {
            ...
            case "file":
                ...
                const opt: { executable?: boolean | string; unzip?: { file: Dir; exclude?: string[] } } = { executable: obj.executable }
...
export async function assets(index: Artifact) {
    ...
    const assetIndex = (await file.download(index.url, { sha1: index.sha1, size: index.size })).toJSON<AssetIndex>()
    ...
    if (assetIndex.map_to_resources) {
        const addIn = (path: string | number, sck: { hash: string; size: number; }) => {
            if (!assetIndex[path]) {
...
export async function getRuntimeIndexes(manifest: RuntimeManifest) {
    ...
    let platform: "gamecore" | "linux" | "linux-i386" | "mac-os" | "mac-os-arm64" | "windows-x64" | "windows-x86" | "linux-arm64" | "linux-arm32" | "windows-arm64";
    switch (getOS()) {
        case ("windows"):
