	...
	async function readMod(
		...
		try {
			...
			if ((metaFile = d.getFile('quilt.mod.json')).exists()) {
				...
				if (metaInfo.quilt_loader.metadata.icon)
					icon = await getIcon(
						file,
						d,
						metaInfo.quilt_loader.metadata.icon,
					);
				...
				if (metaInfo.quilt_loader.metadata?.contributors?.authors) {
					authors.push(
						...Object.keys(
							metaInfo.quilt_loader.metadata.contributors.authors,
						),
					);
				}
				const depends:
			...
			if ((metaFile = d.getFile('META-INF', 'mods.toml')).exists()) {
				...
				state1Map.set(
					'license',
					(val: string) => (modInfoJson.licence = val),
				);
				state1Map.set(
					'credits',
					(val: string) => (modInfoJson.credits = val),
				);
				state1Map.set(
					'logoFile',
					async (val: string) =>
				...
				state2Map.set(
					'version',
					(val: string) => (modInfoJson.version = val),
				);
				state2Map.set(
					'displayURL',
					(val: string) => (modInfoJson.url = val),
				);
				state2Map.set(
					'updateJSONURL',
				...
				state2Map.set('authors', (val: string) => {
					try {
						modInfoJson.authors = val.startsWith('[')
							? JSON.parse(val)
							: [val];
					} catch {
				...
				for (const plr of PLAYERDATA.ls()) {
					if (plr instanceof File && plr.name.endsWith('.dat')) {
						try {
							...
							console.warn(
								'[GMLL]: Failed to parse player data!',
							);
						}
					}
				}
