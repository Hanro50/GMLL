...
export async function setConfig(config) {
    ...
    if (config) {
        if (!config.files || typeof config.files != Array) {
            mainConfig.files = defConfig.files;
        } else {
            mainConfig.files = {};
...
export async function reload() {
    ...
    mkdir(metaFiles.manifest.folder);
    if (mainConfig.update.includes("vanilla")) {
        ...
        if (r.status == 200) {
            ...
            writeFileSync(metaFiles.manifest.latest, JSON.stringify(json.latest, "\n", "\t"));
            writeFileSync(metaFiles.manifest.vanilla, JSON.stringify(json.versions, "\n", "\t"));
        }
    }
    if (mainConfig.update.includes("fabric")) {
        ...
        if (rg2.status == 200 && rg.status == 200) {
            ...
            writeFileSync(metaFiles.manifest.fabric, JSON.stringify(result, "\n", "\t"));
        }
    }
...
export function getVersions() {
    ...
    readdirSync(metaFiles.manifest.folder).forEach(e => {
        if (e.endsWith("json")) {
            var v = JSON.parse(readFileSync(join(metaFiles.manifest.folder, e)));
